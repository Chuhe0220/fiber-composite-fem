    ...
    def meshingFiberOrientation(self):
        ...
        if(self.getHighestNumber(self.DB,DATABASE_POINT)>=3):
            ...
            for p in range(1,self.getHighestNumber(self.DB,DATABASE_POINT)+1):
                ...
                pointSVFs.append(point.attrs[self.PIList[2].AttributeName])
                                    
            for d, v in itertools.product(set(pointTemperatures), set(pointSVFs)):
                points_dv = []
                pointNumbers_dv = []
                for p in range(len(points)):
                    if d == pointTemperatures[p] and v == pointSVFs[p]:
                        points_dv.append(points[p])
                        pointNumbers_dv.append(pointNumbers[p])
                
                if len(points_dv) < 3:
                    continue
                
                nts=np.array(points_dv)
                checkONEline = True  
                cnt = 2                
                px  = points_dv[0][0]
                py  = points_dv[0][1]
                ...
                dpqx = px - qx
                dpqy = py - qy
                while(checkONEline and cnt<len(points_dv)):
                    mpx = points_dv[cnt][0]
                    mpy = points_dv[cnt][1]
                    dpmx = px - mpx
                    dpmy = py - mpy
